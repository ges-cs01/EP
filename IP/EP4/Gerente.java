...
public class Gerente extends Pessoa implements InterfaceGerente{
	...
	public boolean adicionarCliente(Cliente cliente) {
        //verificação do máximo de clientes.
        if(numClientes == clientes.length) {
            return false;
        }
        
        //verificação de duplicados por cpf.
        ...
        //adiciona o cliente na posição numClientes.
        if (numClientes < clientes.length) {
	...
	public void cobrarTodosEmprestimos() {
        ...
        //laço que percorre por todos os clientes.
        for (int i = 0; i < numClientes; i++) {
            atual = clientes[i];

            //se tal condição for verdadeira o cliente paga e a dívida zerada.
            if (atual.getValorContaCorrente() >= atual.getValorDaDivida()) {
                atual.setValorContaCorrente(atual.getValorContaCorrente() - atual.getValorDaDivida());
                atual.setValorDaDivida(0);
            }

            //se tal condição for verdadeira o cliente paga parte da dívida e a conta corrente zerada.
            if (atual.getValorContaCorrente() < atual.getValorDaDivida()) {
                atual.setValorDaDivida(atual.getValorDaDivida() - atual.getValorContaCorrente());
                atual.setValorContaCorrente(0);
           }
        }
    }
}
