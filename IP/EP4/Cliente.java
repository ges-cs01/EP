...
public class Cliente extends Pessoa implements InterfaceCliente{
	...
	public boolean obterEmprestimo(int valor) {
		//verifica se o valor do empréstimo pertence ao conjunto dos naturais.
        if (valor <= 0) {
            return false;
        }

        //verifica se o cliente está apto a obter empréstimo de acordo com o máximo estabelecido.
        if ((valor + getValorDaDivida()) > dividaMaxima) {
            return false;
        }

        //atualiza os valores da dívida e da conta corrente de acordo com o valor do empréstimo.
        else {
	...
	public boolean pagarEmprestimo(int valor) {
		//verifica se o valor pago pertence ao conjunto dos naturais.
	    if (valor <= 0) {
            return false;
        }
        
        //verifica valores não excedentes de pagamento.
        else if (valor > valorDaDivida || valor > valorContaCorrente) {
            return false;
        }

        //atualiza os valores da dívida e da conta corrente de acordo com o pagamento.
        else {
	...
	public boolean realizarSaque(int valor) {
		//verifica se o valor do saque pertence ao conjunto dos naturais.
        if (valor <= 0) {
            return false;
        }

        //verifica se o valor total do saque está presente na conta.
        else if (valor > valorContaCorrente) {
            return false;
        }

        //saque é realizado de acordo com o valor.
        else {
            valorContaCorrente = valorContaCorrente - valor;
            return true;
        }
